...
public class Project {
    ...
    private static void solve() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            int start = ni(), end = ni(), reward = ni();
            arr.add(new int[]{start, end, reward});
        }
        Collections.sort(arr, (a, b) -> a[0] - b[0]);
        //dp + binary search
        //if you chose a project, you need to find the minimum idx whose start date is after the end date of the chosen project
        for (int i = n - 1; i >= 0; i--) {
            long not_pick = dp[i + 1];
            int next = search(arr.get(i)[1]);
            long pick = 0;
            if (next <= n)
                pick = arr.get(i)[2] + dp[next];
