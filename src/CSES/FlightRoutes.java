...
public class FlightRoutes {
    ...
    public static void main(String[] args) throws Exception {
        /*
         * - starting to believe dij can solve any type of shortest path questions
         * - in this instead of dist array being a long array, each entry is a pq for a certain node
         * - describing the k shortest distances for that node
         * - you keep adding the distances if nodepq.size() < k or if it is k, check if the largest among stored distances is greater than the current one
         */
        int t = 1;
        while (t-- > 0) {
            solve();
    ...
    private static void solve() throws IOException {
        ...
        for (int i = 0; i < m; i++) {
            int u = ni() - 1, v = ni() - 1, price = ni();
            g.get(u).add(new int[]{v, price});
        }
        PriorityQueue<Long>[] dist = new PriorityQueue[n];
        for (int i = 0; i < n; i++) {
        ...
        while (!pq.isEmpty()) {
            ...
            long d = node[1];
            for (int[] to : g.get(u)) {
                int v = to[0], price = to[1];
                PriorityQueue<Long> node_pq = dist[v];
                if (node_pq.size() == k && node_pq.peek() <= d + price)
                    continue;
                if (node_pq.size() < k)
                    node_pq.add(d + price);
                else if (node_pq.peek() > d + price) {
                    node_pq.poll();
                    node_pq.add(d + price);
                }
                pq.add(new long[]{v, d + price});

            }

        }
        PriorityQueue<Long> res = dist[n - 1];
