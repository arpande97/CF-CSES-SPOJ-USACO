...
public class TwoSetsII {
    ...
    private static void solve() throws IOException {
        /*
         * - there is a very neat trick here to not calculate the same subsets twice. when recursing, you are counting if one set amounts to half of total
         * - but this way you will calculate complementary subset also
         * - so recurse only till n - 1 -> ensuring you always put the last element in the second set thereby counting the subset just once
         */
        int n = ni();
        int total = (n * (n + 1))/2;
        if (total % 2 != 0) {
        ...
        for (int i = n - 1; i >= 1; i--) {
            for (int acc = req - 1; acc >= 0; acc--) {
                dp[i][acc] = dp[i + 1][acc];
                if (acc + i <= req)
                    dp[i][acc] = (dp[i][acc] + dp[i + 1][acc + i]) % IMOD;
            }
        }
        
        println(dp[1][0]);
    }
    // private static int solve(int num, int acc, int req, int n) {
    //     if (acc == req)
    //         return 1;
    //     if (acc > req || num >= n)
    //         return 0;
    //     if (dp[num][acc] != -1)
    //         return dp[num][acc];
    //     return dp[num][acc] = (solve(num + 1, acc, req, n) + solve(num + 1, acc + num, req, n)) % IMOD;
    // }
