...
public class FlightDiscount {
    ...
    /*
     * - because you can half the price at some point -> the answer is not simply is the shortest route and then halving the 
     * - greatest price. take f.e -> a is connected to b by a price of 10 using one edge and then using 7 edges of price 1
     * - dij will give 7 as the answer, halving one of the edge will reduce price to 6 but in the longer route of 10, halving
     * - gives 5 which is ultimately shorter
     * - maybe explore all paths from 1 to n using dfs and for each path:
     *      - calculate the price to it
     *      - also calculate the max price of any edge in this path
     * - there is a better way: state dij, use a var to store if you have taken a halved edge till now
     * - usually in dij questions where you need to store additional state other than the distance to the node, 
     * - the dimension of dist will also increase
     * 
     */
    ...
    private static void solve() throws IOException {
        ...
        for (int i = 0; i < m; i++) {
            ...
            g.get(u).add(new int[]{v, c, 0});
            g.get(u).add(new int[]{v, c/2, 1});//0 - og edge, 1 - halved edge
        }
        ...
        pq.add(new long[]{0, 0, 0});//node, dist, isHalfEdgeHasBeenUsed
        while (!pq.isEmpty()) {
            long[] node = pq.poll();
            long d = node[1];
            int u = (int)node[0], isHalf = (int)node[2];
            if (dist[u][isHalf] < d)
                continue;
            for (int[] to : g.get(u)) {
                int v = to[0], c = to[1], half = to[2];
                if (half + isHalf == 2)
                    continue;
                if (dist[v][half + isHalf] > d + c) {
                    dist[v][half + isHalf] = d + c;
                    pq.add(new long[]{v, d + c, half + isHalf});
                }
            }
        }
        println(Math.min(dist[n - 1][0], dist[n - 1][1]));
    }
