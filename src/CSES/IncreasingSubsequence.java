    ...
    private static void solve() throws IOException {
        ...
        for (int i = 1; i < n; i++) {
            if (arr[i] > tail.get(tail.size() - 1))
                ...
                int idx = search(arr[i], arr, tail);
                tail.set(idx, arr[i]);
            }
        }
        println(tail.size());
    }
    //1 2 5 7 8 you want to insert 6, so you are looking for the ceil of the number
    private static int search(int val, int[] arr, List<Integer> tail) {
