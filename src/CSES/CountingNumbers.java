...
public class CountingNumbers {
    ...
    private static void solve() throws IOException {
        long a = nl(), b = nl();
        String B = String.valueOf(b), A = String.valueOf(a - 1);
        int b_len = B.length(), a_len = A.length();
        dp = new Long[b_len][2][2][10];
        long R = solve(B, 0, 1, 0, '0');
        dp = new Long[a_len][2][2][10];
        long L = solve(A, 0, 1, 0, '0');
        println(R - L);
    }
    //edge represents if we put the same digit as the range or not at pos
    private static long solve(String str, int pos, int edge, int started, char last) {
        if (pos == str.length())
            return 1;
        if (dp[pos][edge][started][last - '0'] != null)
            return dp[pos][edge][started][last - '0'];
        char limit = edge == 1 ? str.charAt(pos) : '9';
        long count = 0;
        for (char ch = '0'; ch <= limit; ch++) {
            if (started == 1 && ch == last)
                continue;
            int e = ch == limit && edge == 1 ? 1 : 0;
            int s = ch == '0' && started == 0 ? 0 : 1;
            count += solve(str, pos + 1, e, s, ch);
        }

        return dp[pos][edge][started][last - '0'] = count;
