...
public class HotelQueries {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < m; i++) {
            int rooms = ni();
            int res = query(0, n - 1, 1, tree, rooms);
    ...
    private static int query(int l, int r, int node, int[] tree, int val) {
        if (tree[node] < val) {
            return -1;
        }
        if (l == r) {
        ...
        int mid = l + (r - l)/2;
        int res = query(l, mid, 2 * node, tree, val);
        if (res == -1) {
            res = query(mid + 1, r, 2 * node + 1, tree, val);
        }
    ...
    private static void build(int l, int r, int node, int[] tree) {
        if (l == r) {
            tree[node] = arr[l];
            return;
        }
        int mid = l + (r - l)/2;
        build(l, mid, 2 * node, tree);
        build(mid + 1, r, 2 * node + 1, tree);
        tree[node] = Math.max(tree[2 * node], tree[2 * node + 1]);
        return;
    }
