...
public class C1946 {
    ...
    private static boolean valid(int x, int k, int n) throws IOException {
        e = k;
        sub = new int[n];
        dfs(0, -1, x);
        return e == 0 && sub[0] >= x;
    }
    private static void dfs(int u, int parent, int x) {
        sub[u] = 1;
        for (int v : g.get(u)) {
            if (v == parent)
                continue;
    ...
    private static void unionBySize(int x, int y, int[] parent, int[] size) {
        x = find(x, parent);
        y = find(y, parent);
        if (size[x] < size[y])
            unionBySize(y, x, parent, size);
        parent[y] = x;
        size[x] += size[y];
    }
