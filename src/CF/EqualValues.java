...
public class EqualValues {
    ...
    /*
     * cost to make all elements left of i equal to a[i] -> (i - 1) * a[i]
     * to right -> (n - i) * a[i]
     * -one check where all elements are already equal (cost = 0)
     * - cases
     *      - if array is already equal to left or right of some i -> 1 2 3 4 1 1 1 1 cost here would be 4
     *      - but you can't always chose the first i that is equal in the above case -> 1 2 2 20 20 20 20
     * - you are supposed to make everything equal what does that tell you?
     * - how about I start treating continuous integers which are equal. suppose integer x is equal for a range (l, r)
     * - then cost is (l - 1) * x + (n - r) * x, you reduce the cost because chosing i as anything in between (l, r) is a waste
     * - one weird case is [1, 4, 3] in this case according to question you can't chose the first index for op
     * -technically answer for above should be 2, but I'm guessing its going to be 6, so handle for i = 1(0 indexed separately by checking if you even need to change left range)
     */
    private static void solve() throws IOException {
        ...
        for (int i = 0; i < n;) {
            int val = arr[i];
            long left = (long)i * arr[i];
            i++;
            while (i < n && arr[i] == arr[i - 1]) {
                i++;
            }
            long right = (n - i) * (long)val;
