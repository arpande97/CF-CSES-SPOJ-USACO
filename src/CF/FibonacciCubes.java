...
public class FibonacciCubes {
    ...
    private static void solve() throws IOException {
        /*
         * - this is stupid all you need to realize if the box should fit the biggest box (all the sides(l, w, h) should be greater or equal to fib(n))
         * - secondly, the second last biggest box should fit, so one of the sides of the box should be greater than fibn + fibn-1
         * -then everything takes care of itself, because fibn = fibn - 1 + fibn - 2, so you can keep putting boxes
         */
        int n = ni(), m = ni();
        int fibBig = fib[n], fibSmall = fib[n - 1];
        for (int i = 0; i < m; i++) {
            int a = ni(), b = ni(), c = ni();
            boolean c1 = a >= fibBig && b >= fibBig && c >= fibBig;
            if (!c1)
                print('0');
            else {
                boolean c2 = a >= fibBig + fibSmall || b >= fibBig + fibSmall || c >= fibBig + fibSmall;
                if (c2)
                    print('1');
                else
                    print('0');
