...
public class KireiAttacks {
    ...
    private static void solve(List<List<Integer>> g, int[] danger, int n) throws IOException {
        long[] threat = new long[n];
        dfs(0, 0, 0, -1, 0, danger, g, threat);
        for (long t : threat) {
            printSp(t);
        }
        println();
    ...
    private static void dfs(int node, long msf0, long msf1, int parent, int parity, int[] danger, List<List<Integer>> g, long[] threat) {
        threat[node] = danger[node] + (parity == 0 ? msf0 : msf1);
        long cmsf0 = parity == 0 ? danger[node] + msf0 : (-danger[node] + msf0 >= 0 ? msf0 - danger[node] : 0);
        long cmsf1 = parity == 1 ? danger[node] + msf1 : (-danger[node] + msf1 >= 0 ? msf1 - danger[node] : 0);
        for (int v : g.get(node)) {
            if (v != parent) {
                dfs(v, cmsf0, cmsf1, node, parity ^ 1, danger, g, threat);
            }
        }
    }
